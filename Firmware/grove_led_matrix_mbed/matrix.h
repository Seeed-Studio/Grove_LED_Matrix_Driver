...
public:
    ...
    void requestUpdate() { needUpdate = 1; }

    uint32_t *getRedBuffer() { return draw_buf_red; }

    uint32_t *getGreenBuffer() { return draw_buf_green; }
     ...
     */
    void drawImage(uint16_t xoffset, uint16_t yoffset, uint16_t width, uint16_t height, const uint8_t *image, uint32_t *buf);
